...
async def phone_health(org: str = None) -> dict:
    ...
    for coll, field in PHONE_FIELDS:
        ...
        if org:
            q["org_id"] = org
        rows = await db[coll].find(q, {"_id": 0, "id": 1, "org_id": 1, field: 1, "name": 1}).to_list(5000)
        fixable, unfixable, clash = 0, [], 0
        for r in rows:
            raw = r.get(field)
            if not raw:
                continue
            ...
            if not norm or norm == raw:
                unfixable.append({"id": r.get("id"), "name": r.get("name"), "phone": raw})
                continue
            if await db[coll].find_one({"org_id": r.get("org_id"), field: norm,
                                        "id": {"$ne": r.get("id")}}, {"_id": 1}):
                clash += 1
            else:
                fixable += 1
        out.append({"collection": coll, "field": field, "pending": fixable, "duplicate": clash,
                    "invalid": len(unfixable), "invalid_samples": unfixable[:10]})
